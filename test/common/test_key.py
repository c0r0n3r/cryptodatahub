...
class TestPublicKeyX509(unittest.TestCase):
    @staticmethod
    def _get_public_key_pem(public_key_file_name):
        public_key_path = (pathlib.PurePath(__file__).parent / 'certs') / (public_key_file_name + '.pem')
        with open(str(public_key_path), 'r') as pem_file:
            return pem_file.read()

    @staticmethod
    def _get_public_key_x509(public_key_file_name):
        return PublicKeyX509Base.from_pem(TestPublicKeyX509._get_public_key_pem(public_key_file_name))
    ...
    def test_digest(self):
        public_key_x509 = self._get_public_key_x509('expired.badssl.com')
        self.assertEqual(
            PublicKeyX509Base.get_digest(Hash.MD5, public_key_x509.key_bytes),
            bytes_from_hex_string('67:34:4E:61:C0:43:1C:F1:F7:25:7C:1D:6D:E7:A7:85', ':')
        )

        with self.assertRaises(NotImplementedError) as context_manager:
            PublicKeyX509Base.get_digest(Hash.SHA2_512, public_key_x509.key_bytes)
    ...
    def test_der(self):
        public_key_x509 = self._get_public_key_x509('expired.badssl.com')
        self.assertEqual(public_key_x509, PublicKeyX509Base.from_der(public_key_x509.key_bytes))
